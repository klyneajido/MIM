...
export default function HomeClient({ memes }: { memes: Meme[] }) {
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState("All");

  const filtered = useMemo(() => {
    return filterMemes(memes, search, category);
  }, [memes, search, category]);
    ...
    <main className="min-h-screen bg-black text-white">
      <section className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-12 sm:px-6 lg:px-8">
        <div className="space-y-4">
          <Badge
            className="w-fit px-4 py-2 uppercase tracking-[0.2em]"
            variant="default"
          >
            Meme Reply Engine
          </Badge>
